...
#[doc = "Field `MON_CNT` reader - Read-only register that increments by 1 every time it is read. It is reset to 0 on a VBAT Power On Reset."]
pub type MON_CNT_R = crate::FieldReader<u32, u32>;
#[doc = "Field `MON_CNT` writer - Read-only register that increments by 1 every time it is read. It is reset to 0 on a VBAT Power On Reset."]
pub type MON_CNT_W<'a, const O: u8> = crate::FieldWriter<'a, u32, MCNT_LO_SPEC, u32, u32, 32, O>;
impl R {
    ...
    pub fn mon_cnt(&self) -> MON_CNT_R {
        MON_CNT_R::new(self.bits)
    }
}
impl W {
    #[doc = "Bits 0:31 - Read-only register that increments by 1 every time it is read. It is reset to 0 on a VBAT Power On Reset."]
    #[inline(always)]
    pub fn mon_cnt(&mut self) -> MON_CNT_W<0> {
        MON_CNT_W::new(self)
    }
