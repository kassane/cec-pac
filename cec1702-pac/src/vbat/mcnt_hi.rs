...
#[doc = "Field `CNT_HWRD` reader - Thirty-two bit read/write register. If software sets this register to an incrementing value, based on an external\n non-volatile store, this register may be combined with the Monotonic Counter Register to form a 64-bit monotonic counter."]
pub type CNT_HWRD_R = crate::FieldReader<u32, u32>;
#[doc = "Field `CNT_HWRD` writer - Thirty-two bit read/write register. If software sets this register to an incrementing value, based on an external\n non-volatile store, this register may be combined with the Monotonic Counter Register to form a 64-bit monotonic counter."]
pub type CNT_HWRD_W<'a, const O: u8> = crate::FieldWriter<'a, u32, MCNT_HI_SPEC, u32, u32, 32, O>;
impl R {
    ...
    #[inline(always)]
    pub fn cnt_hwrd(&self) -> CNT_HWRD_R {
        CNT_HWRD_R::new(self.bits)
    }
}
impl W {
    #[doc = "Bits 0:31 - Thirty-two bit read/write register. If software sets this register to an incrementing value, based on an external\n non-volatile store, this register may be combined with the Monotonic Counter Register to form a 64-bit monotonic counter."]
    #[inline(always)]
    pub fn cnt_hwrd(&mut self) -> CNT_HWRD_W<0> {
        CNT_HWRD_W::new(self)
    }
